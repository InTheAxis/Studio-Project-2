...
RigidBody::RigidBody()
{
	...
	this->s = u = v = a = 0;
	this->staticCoeff = maxStaticFriction = kineticFirction = 0;
	this->mass = 0;
...
void RigidBody::CreateRigidBody(Vector3 forward, float mass, float staticCoeff, float kineticCoeff)
{
	...
	this->kineticFirction = kineticCoeff * GRAVITY * mass;
...
void RigidBody::UpdateSuvat(double dt)
{
	...
	{
		this->a = (forceForward - kineticFirction) / mass;
	}
	else if (forceForward == 0) //means no pedal, friction shld kick in to slow down car
	{
		...
		if (Math::FAbs(v) > 0.1f)
		{
			this->a = (-1 * direction * kineticFirction) / mass;
		}
		else
		{
			u = v = a = 0;
